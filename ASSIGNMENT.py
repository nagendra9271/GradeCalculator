...
def marksValidation(no_of_subjects,exam_type,my_entries,marks_valid,marks_s) :
    flag=0
    lowest=0
    highest=10
    labels=[]
    ...
    if flag==1:
        ...
        for marks_valid in marks_valid:
            marks_valid.config(text="")
        marks_s[0].config(state="disabled")
        studentdata.saveMarks(studentID,exam_type,sub_marks)
...
def assignmentUI(root,exam_type,ID,flag,marks_list):
    studentID.clear()
    studentID.extend(ID)
    my_entries=[]
    marks_valids=[]
    ...
    marks_exam_label=Label(marks_frame,text=f"{exams[2]} MARKS",font=("Arial",13))
    ...
    marks_save=Button(marks_frame,text="SAVE ",command=lambda:marksValidation(no_of_subjects,exam_type,my_entries,marks_valids,marks_s),width=20,height=2,font=("Arial",13))
    marks_save.grid(row=no_of_subjects+5,column=0,padx=50,pady=50)
    marks_s.append(marks_save)
